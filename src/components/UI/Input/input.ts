// Универсальная функция валидации поля по его имени
export function validateField(
  field: HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement
): boolean {
  const name = field.name;
  const value = field.value;
  let regex: RegExp | undefined; // Регулярное выражение для проверки
  let errorMsg = ""; // Сообщение об ошибке
  let isValid = true;
  ...
  switch (name) {
    ...
    default:
      // Для остальных полей валидация не требуется
      return true;
  }

  // Если для поля определено регулярное выражение, проверяем значение
  if (regex && !regex.test(value)) {
    isValid = false;
  }

  // Находим элемент с сообщением об ошибке внутри родительского элемента
...
// Обработчик события blur для отдельного элемента формы
